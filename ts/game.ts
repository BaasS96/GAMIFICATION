...
var currentqgroup : QuestionGroup;
...
export function openQGroup() {
    ...
    for (var i = 0; i < qs.length; i++) {
        ...
        if (groupdata.certificates[this.id]) {
            //Question answered
            let cert : Object = groupdata.certificates[this.id];
...
function createQuestionSimple(question : Question, answered : boolean) {
...
function createQuestionContents(holder : HTMLElement, question : Question) : HTMLDivElement {
    ...
    if (question.qtype === "text") {
        ...
        for (var i = 0; i < question.answers.length; i++) {
            ...
            let radio = <HTMLInputElement>raw.getElementById("qanswer_mc").cloneNode();
...
function checkAnswer(data : {q : Question, f : HTMLElement}) {
    let question : Question = data.q;
...
function submitAnswer(question : Question, answer : any) {
    var data = {
        game: gamedata.id,
        group: groupdata.id,
